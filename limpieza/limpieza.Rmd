...
# Set the file path
file_path <- "../data/anillamiento_de_Manolo.xlsx"

# Read the "DATOS" sheet from the Excel file
anillamiento_de_Manolo <- read_excel(file_path, sheet = "DATOS")
```


```{r}
# Combine DIA, MES, and AÑO columns and format as DD/MM/YYYY
anillamiento_de_Manolo$FECHA <- as.Date(paste(anillamiento_de_Manolo$DIA, anillamiento_de_Manolo$MES, anillamiento_de_Manolo$AÑO, sep = "/"), format = "%d/%m/%Y")

# Print the head of the dataframe to verify the new column
head(anillamiento_de_Manolo,10)
```
...
na_count <- count_na(anillamiento_de_Manolo)
print(na_count)

na_rows <- anillamiento_de_Manolo[!complete.cases(anillamiento_de_Manolo), ]

anillamiento_de_Manolo$SEXO[is.na(anillamiento_de_Manolo$SEXO)] <- -1
...
# Set the file path
file_path <- "./limpio/anillamiento_de_Manolo.csv"

# Write the dataframe to an Excel file
write_csv(anillamiento_de_Manolo, file_path)
...
distance_between <- function(location1, country1, location2, country2) {
  ...
  register_google("AIzaSyCwSrahOYQ9ryCj4-fBVLM7lf_AkOu923c")
